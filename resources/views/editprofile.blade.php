    ...
    <div class="card" style="width: 36rem; margin: 0px auto 50px auto; border:3px solid #439A97; border-radius:20px">
        ...
        <div class="card-body">
            <form action="/editprofile" method="POST" style="border:none;">
                @csrf
                <a href="/editpassword" class="btn btn-primary" style="margin-left: 68%; margin-bottom: 50px;">Edit Password
                    ›</a>
                <div class="form-group">
                    <label class="lbl" for="name">Name</label>
                    <input type="text" class="form-control" id="name" name="name"
                        value="{{ Auth::user()->name }}" onkeydown="show('save_btn')">
                </div><br>
                <div class="form-group">
                    <label class="lbl" for="email">Email</label>
                    <input type="email" class="form-control" id="email" name="email"
                        value="{{ Auth::user()->email }}" onkeydown="show('save_btn')">
                </div><br>
                <div class="form-group">
                    <label class="lbl" for="gender">Gender</label><br>
                    <div class="form-check form-check-inline">
                        <input type="radio" class="form-check-input" name="gender" id="female" value="Female"
                            @if (Auth::user()->gender == 'Female') checked @endif onclick="show('save_btn')">
                        <label class="form-check-label" for="gender">Female</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input type="radio" class="form-check-input" name="gender" id="male" value="Male"
                            @if (Auth::user()->gender == 'Male') checked @endif onclick="show('save_btn')">
